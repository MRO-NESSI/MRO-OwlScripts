...
def moveFile(f):
	...
	ensureDirectory("/home/jcrawford/Images/{}/".format(dateString))
	newName = "/home/jcrawford/Images/{}/{}".format(dateString, os.path.basename(os.path.normpath(f)))
	os.rename(f, newName)
	return newName
...
def subtract(first, second, original):
	...
	dateString = datetime.now().strftime("%Y-%m-%d")
	oname = os.path.basename(os.path.normpath(original))
	second.writeto("/home/jcrawford/Images/{}/subt-{}".format(dateString, oname)) 
...
if __name__== '__main__':
	...
	files = glob.glob("/home/jcrawford/NewImages/*")
	for f in files:
